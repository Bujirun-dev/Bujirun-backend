...
별도 API 호출 없이, 기존 조회 API 응답에 인증 여부 필드가 이미 들어갑니다. **응답 JSON의 실제 키 이름이 API마다 다르니 주의하세요** (직접 호출해서 확인한 값 기준):

| API | 실제 JSON 키 |
|-----|-----------|
| `GET /api/spots/search` | 각 항목의 `visited` (`collected`도 동일 패턴) |
| `GET /api/spots/{spotId}` | `visited` (`collected`도 동일 패턴 — `isVisited`/`isCollected`가 **아님**) |
| `GET /api/itineraries/{id}` | 각 Day → Item → `spot.isVisited` (`isCollected`도 동일 패턴 — 여긴 `is` 접두어가 그대로 붙음) |

> 왜 다른가: `GET /api/spots/{spotId}`는 Lombok 클래스라 Jackson이 `isVisited()` getter를 관례상 `visited`로 벗겨서 직렬화하고, 나머지 둘은 record라 필드명이 그대로 나갑니다. 프론트에서 헷갈리기 쉬운 부분이라 응답 예시 그대로 옮겨뒀습니다 — 값을 읽을 때 위 표의 실제 키로 매핑해주세요.
