...
/**
 * 공식 추천 관광지(부산명소정보 매칭) 배치(matchOfficialRecommendedSpots) 실행 상태를 메모리에 보관.
 * 서버 재시작 시 초기화됨 (단발성 확인 용도).
 */
@Component
@Getter
public class OfficialRecommendedStatusHolder {

    private volatile MigrationStatus status = MigrationStatus.IDLE;
    private volatile MigrationService.OfficialRecommendedResult lastResult;
    ...
    public void markCompleted(MigrationService.OfficialRecommendedResult result) {
